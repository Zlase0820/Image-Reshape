...
//				out_cpu_data[channels * x + 2 + y*out_cols*channels] = src_cpu_data[i_src_col * channels + 2 + i_src_row * cols * channels];
			...
			else
			{
				int zla[3] = { 0,0,0 };
				int avr[3] = { 0,0,0 };
				for (int cpu_temp = 0; cpu_temp < 3; cpu_temp++)
				{
					int rgb[4];
					int max = 0;
					int min = 255;
					rgb[0] = src_cpu_data[i_src_col * channels + cpu_temp + i_src_row * cols * channels];   //左上角
					rgb[1] = src_cpu_data[(i_src_col+1) * channels + cpu_temp + i_src_row * cols * channels];   //右上角
					rgb[2] = src_cpu_data[i_src_col * channels + cpu_temp + (i_src_row+1) * cols * channels]; //左下角
					rgb[3] = src_cpu_data[(i_src_col+1) * channels + cpu_temp + (i_src_row + 1) * cols * channels];  //右下角
					for (int aa = 0; aa < 4; aa++)
					{
						if (rgb[aa] >= max)  max = rgb[aa];
					}
					for (int bb = 0; bb < 4; bb++)
					{
						if (rgb[bb] <= min)  min = rgb[bb];
					}
					zla[cpu_temp] = max - min;
					avr[cpu_temp] = (int)((rgb[0] + rgb[1] + rgb[2] + rgb[3]) / 4);
				}

				if (zla[0] < range  && zla[1] < range && zla[2] < range)
				{
					out_cpu_data[channels * x + y*out_cols*channels] = avr[0];
//					out_cpu_data[channels * x + 1 + y*out_cols*channels] = avr[1];
//					out_cpu_data[channels * x + 2 + y*out_cols*channels] = avr[2];
				}
				else
				{
					for (int m = -1; m <= 2; m++)
...
//		out_gpu_data[channels * x + 2 + y*out_cols*channels] = src_gpu_data[i_src_col * channels + 2 + i_src_row * cols * channels];
	...
	else
	{
		...
		int zla[3] = { 0,0,0 };
		int avr[3] = { 0,0,0 };
		for (int gpu_temp = 0; gpu_temp < 3; gpu_temp++)
		{
			int rgb[4];
			int max = 0;
			int min = 255;
			rgb[0] = src_gpu_data[i_src_col * channels + gpu_temp + i_src_row * cols * channels];   //左上角
			rgb[1] = src_gpu_data[(i_src_col + 1) * channels + gpu_temp + i_src_row * cols * channels];   //右上角
			rgb[2] = src_gpu_data[i_src_col * channels + gpu_temp + (i_src_row + 1) * cols * channels]; //左下角
			rgb[3] = src_gpu_data[(i_src_col + 1) * channels + gpu_temp + (i_src_row + 1) * cols * channels];  //右下角
			for (int aa = 0; aa < 4; aa++)
			{
				if (rgb[aa] >= max)  max = rgb[aa];
			}
			...
			zla[gpu_temp] = max - min;
			avr[gpu_temp] = (int)((rgb[0] + rgb[1] + rgb[2] + rgb[3]) / 4);
		}

		if (zla[0] < range  && zla[1] < range && zla[2] < range)
		{
			out_gpu_data[channels * x + y*out_cols*channels] = avr[0];
//			out_gpu_data[channels * x + 1 + y*out_cols*channels] = avr[1];
//			out_gpu_data[channels * x + 2 + y*out_cols*channels] = avr[2];
		}
		else
		{
			for (int m = -1; m <= 2; m++)
...
//	imwrite("Cubic_CPU_range.jpg", out);    //保存CPU处理后的结果
...
//	imwrite("Cubic_GPU_range.jpg", gpu_out);    //保存GPU处理的结果
